...
export class Edit_productComponent implements OnInit {
  ...
  id = 0
            ...
            if(this.id)
            {
                this.service.getUserDetails(this.id).subscribe(response =>{
                    if(response['status']=='success')
                    {
                        ...
                        this.netweight = user[0].netweight
    ...
    onUpdate() {
        this.service
          .edit_Product(this.name, this.price, this.discount, this.priceWithDiscount, this.netweight, this.mgfdate, this.expiredate,this.description,this.categoryid, this.id)
          .subscribe(response => {
            if (response['status'] == 'success') {
